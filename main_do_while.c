#include<LPC21xx.h>
#include "lcd.h"
#include "kpm.h"
#include "delay.h"
#include "types.h"
#include "edit.h"
#include "rtc.h"
#define DEVICE 19
extern u32 i_flag;
u32 k_flag;
u32 curpass,depass=1234;
u32 ONH=9,ONM=15,OFFH=20,OFFM=15;
u32 h,mi,s,da,dat,mn,y;
void Enable_int(void);
main()
{
	u32 i;
	Init_KPM();
	Init_LCD();
	RTC_Init();
	Enable_int();
	IODIR0|=15<<DEVICE;
//	SetRTCDateInfo(5,10,2025);
//	SetRTCDay(2);
	//SetRTCTimeInfo(4,30,0);
	IOPIN0=IOPIN0&~(15<<DEVICE)|(0xF<<DEVICE);
	while(1)
	{
		i_flag=0,k_flag=1;
		do
		{
			//SetRTCDateInfo(dat,mn,y);
			//SetRTCDay();
		//	SetRTCTimeInfo(4,30,0);
		//	GetRTCDay(&dow);
		 	GetRTCTimeInfo(&h,&mi,&s);
			DisplayRTCTime(h,mi,s);
			GetRTCDateInfo(&dat,&mn,&y);
			DisplayRTCDate(dat,mn,y);
			GetRTCDay(&da);
			DisplayRTCDay(da);
			 k_flag=ColScan();
		}while((i_flag==0)&&(k_flag==1)); 
		if(k_flag==0)
		{
		    CmdLCD(0x01);
	        StrLCD("enter password");
			 password(&curpass);
				if(curpass==depass)
				{ 
					//while(ColScan()==0);
					CmdLCD(0x01);
					StrLCD("validating");
					//while(ColScan()==0);
					for(i=11;i<16;i++)
					{
						CharLCD('.');
						delay_ms(60);
				}
				CmdLCD(0x01);
				StrLCD("valid password");
				delay_ms(2000);
				CmdLCD(0x01);
				if((HOUR>=ONH)&&(HOUR<=OFFH))
				{
					if((MIN>=ONM)&&(MIN<=OFFM))
					{ 
					IOPIN0=IOPIN0&~(15<<DEVICE)|(0x00<<DEVICE);
					}
				}
			}
		}
		if(i_flag==1)
		{
		i_flag=0;
		//CmdLCD(0x01);
		edit_menu();
		}
		CmdLCD(0x01);
	}
}
